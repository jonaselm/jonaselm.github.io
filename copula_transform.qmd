...
The copula of $X$ is a d-dimensional CDF on $[0,1]^d$ with uniform marginals that captures all of the dependence (but not the scale) between $X$ and $G$:

$C_X(u_1, \dots, u_d) = F_X(F^{-1}_{X_1}(x_1), \dots, F^{-1}_{X_d}(x_d))$.
...
3. Transform each observation $U_i$ into $\tilde G_i$ by pushing each marginal through the appropriately scaled inverse Gaussian CDF/erf: $\tilde G_{ij} = 2\gamma_j\text{erf}(2*U_{ij}-1)$.
...
**Lemma**: The stable copula applied to any stable marginals with $\alpha_1 \neq \alpha_2$ cannot be ellptically symmetric. 
