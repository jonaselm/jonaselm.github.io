...
The procedure for the normalized rescaling estimator is as follows. Given a matrix of (stable) observations $\mathbf X = [X_{*,1}, \dots, X_{*,d}] \in \mathbb R^{n \times d}$ with each row representing a distinct realization and each of the $d$ columns a dimension of the dataset: 
