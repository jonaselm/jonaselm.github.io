...
Where $w_i \sim S_{\alpha^{(t)}/2}(\cos \frac{\pi \alpha}{4}^{2/\alpha}, 1, 0)$ for large N. Alternatively, subject to some straightforward regularity conditions, Teimouri et al. suggest swapping the Monte Carlo integration with a series representation for an almost exact evaluation of $f_W(.|\alpha^{(t)})$.
...
Finally, returning to the complete data log likelihood, there's the issue of the second term, which doesn't have a closed form or clear path to maximization. To simplify the process, we use the fact that the $\alpha$ parameter is identical across all of the marginals and a fast numerical MLE already implemented in STABLE to quickly estimate the MLE for this parameter and treat it as fixed. We can either do this for an arbitrary marginal or sample of marginals, or if the dimensonality is small enough, by estimating all marginal $\alpha$ parameters and then averaging them.
...
$\boldsymbol \mu^{(t+1)} = \frac{\sum_{i=1}^n \textbf x_i e_i^{(t,t)}}{\sum_{i=1}^n e_i^{(t,t)}}$, $\Sigma^{(t+1)} = \frac {1}{2n} \sum_{i=1}^n (\textbf x_i - \boldsymbol\mu^{(t)})^\text T (\textbf x_i - \boldsymbol\mu^{(t)}) e_i^{(t,t)}$
