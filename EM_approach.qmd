...
$\boldsymbol \mu^{(t+1)} = \frac{\sum_{i=1}^n \textbf x_i e_i^{(t,t)}}{\sum_{i=1}^n e_i^{(t,t)}}$, $\Sigma^{(t+1)} = \frac {1}{n} \sum_{i=1}^n (\textbf x_i - \boldsymbol\mu^{(t)})^\text T (\textbf x_i - \boldsymbol\mu^{(t)}) e_i^{(t,t)}$
