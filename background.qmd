...
* existing methods in finance
* especially in the high-dimensional setting, we can't expect to estimate a more general dependence structure
* given elliptical symmetry of the data, it's a pretty reasonable model
* etc.
...
If we relax the assumption that variance is finite, then a reasonable model for returns is the stable distribution, which has no closed form pdf in general but has \href{https://en.wikipedia.org/wiki/Multivariate_stable_distribution}{a complicated characteristic function} where the dependence is fully defined by the spectral measure (typically denoted $\Gamma$), a finite measure on the unit circle $\mathbb S^d$.
